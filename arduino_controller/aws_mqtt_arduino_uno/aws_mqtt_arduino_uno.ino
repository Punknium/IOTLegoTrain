...
const char* mqttserver = "192.168.128.4";               // Local Broker
...
void ProcessPayload(char* chararray) {
  ...
  Serial.print("Mode: ");
  Serial.println(root.get<uint8_t>("mode"), DEC);

  if (root.get<uint8_t>("mode") == 0) {
    Serial.println("Setting Train");
    pf.single_pwm(root.get<uint8_t>("channel"), root.get<uint8_t>("value"));
  }
  
  // Channel: %u, Value: %u\n", root["mode"].as<uint8_t>(), root["channel"].as<uint8_t>(), root["value"].as<uint8_t>());
